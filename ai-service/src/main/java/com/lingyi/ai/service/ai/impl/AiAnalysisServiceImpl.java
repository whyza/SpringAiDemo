...
public class AiAnalysisServiceImpl implements AiAnalysisService {
     ...
     * 构建 AI 提示词
     */
    private String buildPrompt(EcommerceDataDTO dataDTO) {
     ...
     * 解析 AI 响应并保存报告
     */
    private DailyReportPushVO buildAndSaveReport(EcommerceDataDTO dataDTO, String aiResponse) {
        Double salesGrowthRate = calculateGrowthRate(dataDTO.getYesterdaySales(), dataDTO.getTodaySales());
        Double revenueGrowthRate = calculateGrowthRate(dataDTO.getYesterdayRevenue(), dataDTO.getTodayRevenue());
        Integer healthScore = calculateHealthScore(dataDTO, salesGrowthRate);

        DailyReportPushVO reportVO = new DailyReportPushVO();
        reportVO.setReportDate(dataDTO.getReportDate() != null
                ? dataDTO.getReportDate()
        ...
        // 调用大模型生成精简版关键报告（用于完整报告展示，不重复亮点/风险/建议）
        try {
    ...
    private String buildDailyReportPrompt(EcommerceDataDTO dataDTO, Double salesGrowthRate, Double revenueGrowthRate) {
        ...
        // 计算客单价
        BigDecimal todayUnitPrice = dataDTO.getTodaySales() != null && dataDTO.getTodaySales() > 0
                ? dataDTO.getTodayRevenue().divide(BigDecimal.valueOf(dataDTO.getTodaySales()), 2, RoundingMode.HALF_UP)
                : BigDecimal.ZERO;
        BigDecimal yesterdayUnitPrice = dataDTO.getYesterdaySales() != null && dataDTO.getYesterdaySales() > 0
                ? dataDTO.getYesterdayRevenue().divide(
                BigDecimal.valueOf(dataDTO.getYesterdaySales()),
                2,
                RoundingMode.HALF_UP
        )
                : BigDecimal.ZERO;
        Double unitPriceGrowth = todayUnitPrice.subtract(yesterdayUnitPrice)
        ...
        double invalidSkuRatio = totalLinksAll > 0
                ? (double) dataDTO.getNoOrderLinks() / totalLinksAll * 100 : 0;
        ...
        return String.format(
                ...
                dataDTO.getTodaySales(), dataDTO.getYesterdaySales(),
                growthSymbol1, Math.abs(salesGrowthRate),
                dataDTO.getTodayRevenue(), dataDTO.getYesterdayRevenue(),
                growthSymbol2, Math.abs(revenueGrowthRate),
                todayUnitPrice, yesterdayUnitPrice,
                unitPriceSymbol, Math.abs(unitPriceGrowth),
                dataDTO.getRisingLinks(), risingRatio,
                dataDTO.getFallingLinks(), 100 - risingRatio,
                dataDTO.getNoOrderLinks(), invalidSkuRatio,
                totalLinksAll, totalLinks
        );
    }

    /**
     * 解析 AI 响应，提取各部分内容
     * 支持新格式：关键指标速览表格、核心结论、业务本质、多维度评估、亮点、风险预警、运营建议
     */
    private void parseAiResponse(String aiResponse, DailyReportPushVO reportVO) {
        String[] lines = aiResponse.split("\n");

        // 提取摘要（取非空的第一行，去掉 Markdown 格式符）
        ...
        // 状态机：当前处于哪个区块
        // 0=关键指标速览，1=核心结论，2=业务本质，3=多维度评估，4=亮点，5=风险预警，6=运营建议
        int section = 0;
        ...
        for (String line : lines) {
            ...
            switch (section) {
                case 1: // 核心结论 → 根据内容判断
                    if (content.contains("⚠️") || isNegative(content)) {
                        risks.add(content);
                    } else {
                        highlights.add(content);
                    }
                    break;
                case 2: // 业务本质 → 加入 highlights
                    highlights.add(content);
                    break;
                case 3: // 多维度 → 根据关键词判断
    ...
    private String extractContent(String line) {
        ...
        // 过滤子标题行（**关键词**： 或 ✅关键词： 格式）
        if (content.matches("^[✅⚠️💡🔴🟢][^\\s]{1,8}[：:]$")
                || content.matches("^[^\\s]{2,15}[：:]$") && !content.contains("：") && !content.contains("。")) {
            // 短标题行（如 "✅ 124 个上涨商品中"）→ 保留
            // 纯标题行（如 "利润结构被快速侵蚀"）→ 过滤
    ...
    private boolean isNegative(String content) {
        return content.contains("下降") || content.contains("下滑")
                || content.contains("风险") || content.contains("恶化")
                || content.contains("浪费") || content.contains("低效")
                || content.contains("不足") || content.contains("失衡")
                || content.contains("侵蚀") || content.contains("亏损")
                || content.contains("劣化") || content.contains("错配");
    }
