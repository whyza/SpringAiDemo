...
public class SmartReportEngineServiceImpl implements SmartReportEngineService {
    ...
    public SmartReportResultVO analyze(SmartReportRequestDTO request) {
        ...
        String diagnosisConclusion = generateDiagnosisConclusion(request);
        String operationDiagnosis = generateOperationDiagnosis(request, diagnosisConclusion);
    ...
    private String generateOperationDiagnosis(SmartReportRequestDTO req, String diagnosisConclusion) {
        String systemPrompt = """
                ...
                3. 必须包含以下六部分：
                核心结论、业务本质、多维度评估、亮点、风险预警、风险预警部分。
                4. “亮点”部分使用“✅”开头逐条输出。
    ...
    private String buildDiagnosisConclusionPrompt(SmartReportRequestDTO req) {
        return String.format(
                """
                        ...
                        ## 判断规则
                        红色规则：
                        1. 销售额暴跌：当销售额变化为负，且下滑幅度大于等于 %s%% 时命中。
                        2. 大量链接滞销：当今日未出单链接数占今日总链接数比例大于等于 %s%% 时命中。

                        黄色规则：
                        1. 销售额小幅下滑：当销售额变化为负，且下滑幅度大于等于 %s%%，同时小于红色销售额阈值时命中。
                        2. 下跌链接偏多：当今日下跌链接数大于今日上涨链接数的 %s 倍时命中。
                        3. 存在滞销链接：当今日未出单链接数大于等于 1，且未达到红色未出单占比阈值时命中。

                        绿色规则：
                        1. 销售额增长：当销售额变化大于等于 %s%% 时命中。
                        2. 爆款涌现：当今日上涨链接数占今日总链接数比例大于等于 %s%% 时命中。
                        """,
    ...
    private String buildFallbackOperationDiagnosis(SmartReportRequestDTO req, String diagnosisConclusion) {
        ...
        content.append("### 核心结论\n");
        content.append("1. 当天销售额 ").append(formatAmount(req.getTodayRevenue())).append(" USD，昨日销售额 ")
                .append(formatAmount(req.getYesterdayRevenue())).append(" USD，销售额变化 ").append(revenueChange).append("。\n");
        content.append("2. 当天订单量 ").append(safeInt(req.getTodayOrders())).append(" 单，昨日订单量 ")
                .append(safeInt(req.getYesterdayOrders())).append(" 单。\n");
        content.append("3. 今日总链接数 ").append(todayTotalLinks).append("，昨日总链接数 ")
                .append(yesterdayTotalLinks).append("。\n\n");

        content.append("### 业务本质\n");
        content.append("1. 当前经营表现主要由销售额、订单量和链接结构共同驱动。\n");
        content.append("2. 今日上涨链接数 ").append(safeInt(req.getTodayRisingLinks())).append("，今日下跌链接数 ")
                .append(safeInt(req.getTodayFallingLinks())).append("，说明链接表现存在分化。\n");
        content.append("3. 今日未出单链接数 ").append(safeInt(req.getTodayNoOrderLinks())).append("，直接反映低效链接压力。\n\n");

        content.append("### 多维度评估\n");
        content.append("1. 销售维度：当天销售额 ").append(formatAmount(req.getTodayRevenue())).append(" USD，昨日销售额 ")
                .append(formatAmount(req.getYesterdayRevenue())).append(" USD，变化 ").append(revenueChange).append("。\n");
        content.append("2. 订单维度：当天订单量 ").append(safeInt(req.getTodayOrders())).append(" 单，昨日订单量 ")
                .append(safeInt(req.getYesterdayOrders())).append(" 单。\n");
        content.append("3. 链接维度：今日上涨 ").append(safeInt(req.getTodayRisingLinks())).append("，今日下跌 ")
                .append(safeInt(req.getTodayFallingLinks())).append("，今日未出单 ")
                .append(safeInt(req.getTodayNoOrderLinks())).append("。\n");
        content.append("4. 对比维度：昨日上涨 ").append(safeInt(req.getYesterdayRisingLinks())).append("，昨日下跌 ")
                .append(safeInt(req.getYesterdayFallingLinks())).append("，昨日未出单 ")
                .append(safeInt(req.getYesterdayNoOrderLinks())).append("。\n\n");
    ...
    private TriggeredRules evaluateFallbackRules(SmartReportRequestDTO req) {
        ...
        if (BigDecimal.valueOf(safeInt(req.getTodayFallingLinks()))
                .compareTo(BigDecimal.valueOf(safeInt(req.getTodayRisingLinks())).multiply(defaultDecimal(req.getY2Ratio()))) > 0) {
    ...
    private BigDecimal calculateRevenueChange(SmartReportRequestDTO req) {
        ...
        return todayRevenue.subtract(yesterdayRevenue)
