...
public class SmartReportEngineServiceImpl implements SmartReportEngineService {
    ...
    private SmartReportResultVO.TriggeredRulesVO evaluateRules(SmartReportRequestDTO req) {
        ...
        // R1: 销售额暴跌
        if (revenueChangeRate.compareTo(BigDecimal.ZERO) < 0 && revenueChangeRate
                .abs()
                .compareTo(req.getR1Threshold()) >= 0) {
            result.getRedAlerts().add("销售额大幅下滑预警");
        }

        // R2: 大量链接滞销
        if (totalLinks > 0) {
            BigDecimal noOrderRatio = BigDecimal
                    .valueOf(safeInt(req.getNoOrderLinks()))
        ...
        // R3: 利润率恶化
        if (req.getProfitRate().compareTo(req.getR3ProfitMin()) < 0 && req
                .getProfitRateChange()
                .abs()
                .compareTo(req.getR3ProfitDrop()) >= 0 && req.getProfitRateChange().compareTo(BigDecimal.ZERO) < 0) {
        ...
        if (revenueChangeRate.compareTo(BigDecimal.ZERO) < 0 && revenueChangeRate
                .abs()
                .compareTo(req.getY1Threshold()) >= 0 && revenueChangeRate.abs().compareTo(req.getR1Threshold()) < 0) {
        ...
        // G2: 爆款涌现
        if (totalLinks > 0) {
            BigDecimal risingRatio = BigDecimal
                    .valueOf(safeInt(req.getRisingLinks()))
                    .multiply(BigDecimal.valueOf(100))
                    .divide(BigDecimal.valueOf(totalLinks), 2, RoundingMode.HALF_UP);
        ...
        // G3: 利润率健康
        if (req.getProfitRate().compareTo(req.getG3Threshold()) >= 0 && req
                .getProfitRateChange()
    ...
    private String generateAiContent(SmartReportRequestDTO req, SmartReportResultVO.TriggeredRulesVO triggered) {
        ...
        String revenueChangeStr = (revenueChange.compareTo(BigDecimal.ZERO) >= 0
                ? "↑"
                : "↓") + revenueChange.abs() + "%";
        String profitChangeStr = (req.getProfitRateChange().compareTo(BigDecimal.ZERO) >= 0 ? "↑" : "↓") + req
                .getProfitRateChange()
                .abs() + "pt";
        ...
        try {
            ...
            log.error("AI 调用失败", e);
            throw new RuntimeException("AI 运营建议生成失败：" + e.getMessage(), e);
        }
    }
